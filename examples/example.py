"""Demonstrate basic usage of the tabx module.
...
try:
    ...
    raise SystemExit("tabx is not installed.\nRun: pip install -e .")

# Single-row API:
numbers = tabx.parse_csv_numbers("10, 20, 30")
total = tabx.sum_csv_numbers("10, 20, 30")
...
flat = tabx.parse_csv_flat(CSV, skip_header=True)
grand = tabx.sum_csv_all(CSV, skip_header=True)
...
df = tabx.parse_csv_dataframe(MULTI_CSV)
print(f"\nparse_csv_dataframe:\n{df}")
